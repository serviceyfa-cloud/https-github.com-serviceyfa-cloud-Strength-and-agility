...
export type OrderShippingStatus = 'pending' | 'in_transit' | 'delivered' | 'returned';
...
/**
 * بنية سجل الطلب (Order Schema Contract)
 * مهيأة معمارياً لاستقبال بيانات الطلبات الحقيقية فور ربط الخادم (Backend Orders API).
 */
...
export const OrdersPage: React.FC = () => {
  // لا توجد سجلات وهمية - المصفوفة فارغة تماماً
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6 text-start" dir="rtl">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs px-2.5 py-0.5 rounded-md font-bold bg-[#EFF6FF] text-[#1257D6] border border-[#BFDBFE]">
              بانتظار اتصال مصدر البيانات
            </span>
          </div>
          <p className="text-xs sm:text-sm text-[#64748B]">
            متابعة ومعالجة طلبات المتجر، وتحديث حالات الدفع والشحن فور اكتمال دورة الشراء.
          </p>
        </div>
      </header>

      {/* تنبيه معماري حول متطلبات ربط الطلبات الحقيقية */}
      <div className="p-4 rounded-xl bg-[#FFFFFF] border border-[#E2E8F0] flex items-start gap-3">
        ...
        <div className="text-xs text-[#475569] leading-relaxed">
          ...
          النظام المعماري مهيأ لاستقبال بيانات الطلبات (معرّف الطلب، بيانات العميل، قائمة المنتجات، الإجمالي المالي، حالة الدفع، حالة الطلب، وحالة الشحن) تلقائياً بمجرد تفعيل واجهة برمجة التطبيقات (Orders API) ونظام قاعدة البيانات.
        </div>
      </div>
      ...
      {/* مساحة العرض الرئيسية: حالة الفراغ النظيفة والمهنية */}
      <Card className="bg-[#FFFFFF] border-[#E2E8F0] rounded-2xl shadow-none">
        <CardContent className="py-20 sm:py-24 px-6 text-center">
          ...
          <div className="inline-flex items-center gap-2 text-xs text-[#64748B] bg-[#F8FAFC] px-4 py-2 rounded-xl border border-[#E2E8F0]">
            <RefreshCw className="w-3.5 h-3.5 text-[#1257D6]" aria-hidden="true" />
            <span>بانتظار اتصال مصدر البيانات لتسجيل أول طلب</span>
          </div>
        </CardContent>
      </Card>
    </div>
